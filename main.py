...
if __name__ == "__main__":
    ...
    for seed in [22, 42, 62, 82, 102]:
        for file_num in range(4):
            ...
            # print(f"# Samples: {len(y_all)}, # Features: {len(feature_names)}")
            # continue
            ...
            for c, data in split_data.items():
                ...
                rf = RandomForestClassifier(n_estimators=best_n, max_depth=best_d, random_state=seed)
                rf.fit(X_train, y_train)
            ...
            # 打印加权平均结果
            print(f"\n===== Random Forest (Cluster = {best_k}, Seed = {seed}) =====")
